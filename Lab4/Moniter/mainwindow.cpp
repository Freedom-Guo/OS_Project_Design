...
void MainWindow::setupRealtimeDataDemo(QCustomPlot *customPlot){
   ...
   // customPlot->graph(0)->setPen(color);
...
void MainWindow::info_update(){
    ...
    ui->label_curtime->setText(currentDate);
   ...
   // qDebug()<<a<<"+"<<b;
    ...
    //setupRealtimeDataDemo(ui->customPlot);
    double key = QDateTime::currentDateTime().toMSecsSinceEpoch()/1000.0;//second
    ...
    //qDebug()<<usge;
    //ui->label->setText(QString::number(usge));
    ui->progressBar->setValue(usge);
    //ui->progressBar->setFormat(QString::fromLocal8Bit("%1%").arg(QString::number(usge,'f',2)));
...
void MainWindow::show_Table_Info(int index){
    ...
    if(index==0){
        ...
        while(flag>0){
            ...
            if(pos!=-1){
                pos+=13;
                QString *cpuname=new QString(tempstring.mid(pos,tempstring.length()-13));
        ...
        tempstring=infoFile.readLine();
        ui->label_hostname->setText(tempstring);
        infoFile.close();
        //systime
        ...
        ui->label_osstart->setText(qstartTime);
...
//        QString oswork=QString::number(worktime);
//        oswork=oswork+"s";
        ui->label_oswork->setText(oswork);
        infoFile.close();
    }
    else if(index==1){
        infoFile.setFileName("/proc/meminfo");
...
//                                                   "\t"+QString::fromUtf8("Mem"),ui->listWidget);
        QStringList header;
        header<<"PID"<<"Name"<<"State"<<"Priority"<<"PPID"<<"Mem";
...
//        ui->tableWidget->setItem(0,2,new QTableWidgetItem("State"));
//        ui->tableWidget->setItem(0,3,new QTableWidgetItem("Priority"));
//        ui->tableWidget->setItem(0,4,new QTableWidgetItem("Mem"));
        int cur_row=0;
        while(1){
            posa=pro.indexOf("\n",find_start);
            posb=pro.indexOf("\n",posa+1);
            ...
            cur_row++;
//           if(nameofpro.length()>=13){
//                QListWidgetItem *item=new QListWidgetItem(pro_id+"\t"+nameofpro+
//                                                           +"\t"+stateofpro+"\t"+
//                                                           priofpro+
//                                                           "\t"+memofpro,ui->listWidget);
//            }
//            else{
//                QListWidgetItem *item=new QListWidgetItem(pro_id+"\t"+nameofpro+
//                                                           +"\t\t"+stateofpro+"\t"+
...
//            }
        ...
        //infoFile.setFileName("/proc");
        //QStringList proslist=;
    else if(index==3){
        ;
    }
    else{
